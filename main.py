...
def _ensure_writable_data_dir(path: Path) -> Path:
    try:
        ...
        try:
            ...
            # Fallback to /tmp which always has write permissions in Linux containers
...
logger.info(f"State file path: {DATA_FILE}")
logger.info(f"Secret key path: {SECRET_FILE}")
